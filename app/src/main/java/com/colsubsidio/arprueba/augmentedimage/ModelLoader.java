...
public class ModelLoader {
  ...
  private final WeakReference<AugmentedImageActivity> owner;

  ModelLoader(AugmentedImageActivity owner) {
    this.owner = new WeakReference<>(owner);
  }

  /**
   * Starts loading the model specified. The result of the loading is returned asynchrounously via
   * {@link AugmentedImageActivity#setRenderable(int, ModelRenderable)} or {@link
   * AugmentedImageActivity#onException(int, Throwable)}.
   *
  ...
  boolean loadModel(int id, int resourceId) {
    AugmentedImageActivity activity = owner.get();
    if (activity == null) {
